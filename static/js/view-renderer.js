...
function allowLayerControl(LayerNro) {
  ...
  if (limitLayers) {
    const limitItems = limitLayers
      .replace("[", "")
      .replace("]", "")
      .replace(" ", "")
      .split(","); // Ugly, but works.
    // Potential bug! FIXME: The value of LayerNro currently MUST be a string. Not integer! ==> Fixed in 1.1.4.
    if (limitItems.includes(String(LayerNro))) {
      // Forced to string since v.1.1.4
...
function clearLayer(LayerNro) {
  ...
  if (!ValidatedLayerNro) {
    console.log(
      "...but forcefully clearing restricted layer " + LayerNro + "."
    );
...
function protocolCheck() {
  ...
  if (protocol != "http:" && protocol != "https:") {
    alert(
      "Please use this via server! Functions and CORS may not work when used as a file. (Current protocol: " +
        protocol +
        ")"
    );
  }
...
function populateLayer(LayerNro, TemplateFileRef = "", forced = false) {
  ...
  document.getElementById(iframe).src =
    templa + "?random=" + ranNro + customParams;
...
socket.on("SPXMessage2Client", function (data) {
  ...
  switch (data.spxcmd) {
      ...
      if (urlParams.get("remote") == "true" && nameMatch) {
        ...
        if (data?.layers?.length > 0) {
          window.location.href =
            baseURL + "?layers=" + layerList + "&remote=true&name=" + userName;
        } else {
    ...
    case "updateTemplate":
      setTimeout(function () {
        updateLayer(data.webplayout, data.fields);
      }, 10);
      break;
    ...
    case "invokeFunction":
      ...
      try {
        ...
        if (data.function) {
          ...
          if (
            typeof document.getElementById("layer" + LayerNro).contentWindow[
              myFunction
            ] === "function"
          ) {
            let paramPreview = "";
            if (myParamets && myParamets != "undefined") {
              paramPreview =
                "First 30 chars of args: " + myParamets.substring(0, 30);
            } else {
            ...
            document
              .getElementById("layer" + LayerNro)
              .contentWindow[myFunction](myParamets);
          } else {
            console.log(
              'Function "' +
                myFunction +
                '" not found on layer ' +
                LayerNro +
                ". Verify function and loaded template."
            );
          }
        } else {
          if (data.invoke) {
            ...
            warning +=
              "`data.invoke` will be deprecated in the next SPX version. Refactor your extensions to ";
            warning +=
              "use `data.function` and `data.params` instead when calling the /gc/playout API or use ";
            warning +=
              "the public API endpoint /api/v1/invokeTemplateFunction instead.\n";
